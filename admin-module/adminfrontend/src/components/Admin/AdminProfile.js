  ...
  const getEmployeeData = async () => {
    try {
      const { data } = await axios.get(
        `http://localhost:8888/api/v1/admin/getEmployeeDetails/${branch}/${user.employee_ID}`,
        {
          headers: {
  ...
  const editEmployeeData = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        `http://localhost:8888//api/v1/admin/editEmployeeDetails/${branch}/${user.employee_ID}`,
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",

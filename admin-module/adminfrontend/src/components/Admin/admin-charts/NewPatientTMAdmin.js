...
const NewPatientTMAdmin = () => {
  ...
  const getAppointList = async () => {
    try {
      ...
      const { data } = await axios.get(
        `https://dentalguru-global-admin.vimubds5.a2hosted.com/api/v1/admin/getPatientDetailsByBranch/${user.branch_name}`,
        {
          headers: {
  ...
  const dailyAppointments = appointmentList.data?.reduce((acc, appointment) => {

...
const bookAppointment = (req, res) => {
  try {
    ...
    console.log(timeZone, 'line number 707 controller');
...
const insertTimelineEvent = (req, res) => {
  try {
    const { type, description, branch, patientId, timeZone } = req.body;

    const date = moment().tz(timeZone).format("DD-MM-YYYY"); // Get today's date in DD-MM-YYYY format
...
const sendWhatsapp = async (req, res) => {
  ...
  const fileUrl = `https://dentalguru-global-receptionist.vimubds5.a2hosted.com/prescription/${mediaFile.filename}`;

...
const AllTests = ({ data }) => {
  ...
  const [searchQuery, setSearchQuery] = useState('');
    ...
    console.log(fetchPatientDetails);
...
//new
  ...
  //     const fullName = patient.patient_name.toLowerCase().trim();
  //     const lowerSearchQuery = searchQuery.toLowerCase().trim();
  //     const formattedDate = moment(patient.created_date).format("DD-MM-YYYY");
  ...
  const filteredPatients = patientDetails?.filter((patient) => {
    const fullName = patient.patient_name.toLowerCase().trim();
    const lowerSearchQuery = searchQuery.toLowerCase().trim();
    const formattedDate = moment(patient.created_date, "DD-MM-YYYY HH:mm:ss").format("DD-MM-YYYY");
  
    return (
      (fullName.includes(lowerSearchQuery) ||
        patient.patient_uhid.toLowerCase().trim().includes(lowerSearchQuery)) &&
      (!dateFilter || formattedDate === moment(dateFilter).format("DD-MM-YYYY"))
    );
  });
...
<tbody>
  ...
  {filteredPatients.map((patient, index) => (
    <>
      ...
      <td>{moment(patient.created_date, "DD-MM-YYYY HH:mm:ss").format("DD-MM-YYYY")}</td>

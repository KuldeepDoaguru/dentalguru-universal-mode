...
function TodayTest() {
  ...
  useEffect(() => {
    // Make API request to fetch today's test data
    const fetchTodayTestData = async () => {
      try {
        ...
        const todayTests = response.data.filter((test) => {
          ...
          return new Date(test.created_date).toLocaleDateString() === today;
        });
        setTestData(todayTests);
      } catch (error) {
        console.error("Error fetching today test data:", error);
      }
    };
      ...
      <div className="row flex-nowrap ">
        ...
        >
          ...
          <div className="container-fluid">
            <h2 className="mx-2 mt-2">Today's Test Data</h2>
                ...
                {testData.map((test, index) => (
                  <tr key={test.id}>
                    ...
                    <td>
                      <div className="">
                        {" "}
                        <Link to={`/final-oral-testing/${test.testid}`}>

...
const PediatricDentalTest = ({ tpid }) => {
  ...
  const getChiefList = async () => {
    try {
      const { data } = await axios.get(
        `https://dentalguru-global-doctor.vimubds5.a2hosted.com/api/doctor/getChiefComplain`
      );
      setChielfList(data);
    } catch (error) {
      console.log(error);
  ...
  const handleSave = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://dentalguru-global-doctor.vimubds5.a2hosted.com/api/doctor/dentalPediatric",
        formData,
        {
          headers: {
            "Content-Type": "application/json",

...
const ViewQuickPrescription = () => {
  ...
  const getPatientDetail = async () => {
    try {
      const res = await axios.get(
        `https://dentalguru-global-doctor.vimubds5.a2hosted.com/api/doctor/getPatientByAppID/${appointId}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
  ...
  const getTreatPrescriptionByAppointId = async () => {
    try {
      const { data } = await axios.get(
        `https://dentalguru-global-doctor.vimubds5.a2hosted.com/api/doctor/getQuickPrescription/${appointId}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
                    ...
                    <tbody>
                      <>
                        <tr>
                          <td>
                            {item.date === null
                              ? getPatientData[0]?.appointment_dateTime?.split(
                                  "T"
                                )[0]
                              : item.date?.split(" ")[0]}
                          </td>

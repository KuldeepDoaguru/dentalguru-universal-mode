...
console.log(user.currentUser.timeZone);
  ...
  const getPatientTreatmentDetails = async (uhid) => {
    try {
      const response = await axios.get(
        `https://dentalguru-global-receptionist.vimubds5.a2hosted.com/api/v1/receptionist/getPatientDeatilsByUhid/${branch}/${uhid}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
  ...
  const handleBookAppointment = async (e) => {
    ...
    if (isSlotAvailable) {
      ...
      try {
        setLoading(true);
        const response = await axios.post(
          "https://dentalguru-global-receptionist.vimubds5.a2hosted.com/api/v1/receptionist/book-appointment",
          newAppointment,
          {
            headers: {
              "Content-Type": "application/json",

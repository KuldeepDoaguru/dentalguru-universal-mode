...
const PrintSecurityAmt = () => {
  ...
  const getBill = async () => {
    try {
      const response = await axios.get(
        `https://dentalguru-global-receptionist.vimubds5.a2hosted.com/api/v1/receptionist/getSinglePatientSecurityAmt/${branch}/${SId}`,
        {
          headers: {
  ...
  const getPatientTreatmentDetails = async (uhid) => {
    try {
      const response = await axios.get(
        `https://dentalguru-global-receptionist.vimubds5.a2hosted.com/api/v1/receptionist/getPatientDeatilsByUhidFromSecurityAmt/${branch}/${uhid}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
                ...
                <tbody>
                  <tr>
                    ...
                    <th scope="col" className="text-end pe-4 pt-4">
                      {" "}
                      {user.currentUser.currencySymbol} {data[0]?.amount || '00'}
                      {".00"}
                    </th>
                  </tr>
                  <tr>
                  ...
                  {/* <tr>
                  <th scope="col" className='' style={{ width: '30%' }}></th>
                  <th scope="col" className='text-start second-th'>Net Payable {" "}{user.currentUser.currencySymbol}</th>
